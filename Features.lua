-- =================================================================================
-- Rioo UI Template for Fish It Hub - Step 43: MANUAL KEY SYSTEM (FIXED + NEW METHOD INTEGRATED)
...
-- Load Rioo UI Library (PAKAI LINK RESMI)
local success, Rioo = pcall(function() return loadstring(game:HttpGet('https://raw.githubusercontent.com/RioYang010101/Project-Script-Fish-It/main/RiooUi.lua'))() end)
if not success then
    warn("Gagal load Rioo library! Error:", Rioo)
    return
end
...
local function SellAllNow() if RemoteSellAllItems then local s,r=pcall(function()RemoteSellAllItems:InvokeServer()end); if s then Rioo:Notify({Title="Sell",Content="OK"})else Rioo:Notify({Title="Sell Err",Content="Invoke? "..tostring(r)})end else Rioo:Notify({Title="Sell Err",Content="Remote?"})end end
local function ToggleAutoSell(s) autoSellActive=s; if s then autoSellLoop=game:GetService("RunService").Heartbeat:Connect(function()if not autoSellActive then return end; if GetFishCount()>=autoSellThreshold then SellAllNow();task.wait(1)end end); Rioo:Notify({Title="AutoSell",Content="ON"})else if autoSellLoop then autoSellLoop:Disconnect();autoSellLoop=nil end; Rioo:Notify({Title="AutoSell",Content="OFF"})end end
local function ToggleBypassPing(s) bypassPingActive=s; pcall(function()settings().Network.IncomingReplicationLag=s and 0 or 0.1 end); Rioo:Notify({Title="Bypass Ping",Content=s and "ON" or "OFF"})end
local function RunMasterFarm(V) autoFishingActive=V; if V then ToggleBypassPing(true); Rioo:Notify({Title="Master Farm",Content="Started",Duration=3,Image=4483362458}); if autoFishingLoop then autoFishingLoop:Disconnect();autoFishingLoop=nil end; autoFishingLoop=task.spawn(function()lastActivity=tick(); while autoFishingActive do local cA={[4]=tick()}; if RemoteChargeRod then pcall(function()RemoteChargeRod:InvokeServer(unpack(cA))end)end; local rA={[1]=math.random(-1,1),[2]=math.random(0,1),[3]=tick()}; if RemoteRequestMinigame then pcall(function()RemoteRequestMinigame:InvokeServer(unpack(rA))end)end; wait(fishingCompleteDelay); if RemoteFishingCompleted then pcall(function()RemoteFishingCompleted:FireServer()end)end; if autoRecastActive then wait(autoRecastDelay); if RemoteChargeRod then pcall(function()RemoteChargeRod:InvokeServer(unpack(cA))end)end; if RemoteRequestMinigame then pcall(function()RemoteRequestMinigame:InvokeServer(unpack(rA))end)end end; if tick()-lastActivity>1140 then pcall(function()game:GetService("VirtualInputManager"):SendMouseMoveEvent(500,500,game)end);lastActivity=tick()end; wait(autoFishingDelay)end end) else if autoFishingLoop then autoFishingLoop=nil end; ToggleBypassPing(false); Rioo:Notify({Title="Master Farm",Content="Stopped",Duration=3,Image=4483362458})end end
local function TeleportBrutal(tCF) local p=game.Players.LocalPlayer; local c=p.Character; if not c or not c:FindFirstChild("HumanoidRootPart")or not c:FindFirstChildOfClass("Humanoid")then Rioo:Notify({Title="TP Err",Content="Char?"}); return end; local hrp=c.HumanoidRootPart; local h=c:FindFirstChildOfClass("Humanoid"); pcall(function()h:ChangeState(Enum.HumanoidStateType.Physics)end); local s,e=pcall(function()hrp.Anchored=true; c:SetPrimaryPartCFrame(tCF); local st=tick(); while tick()-st<1 do if not hrp or not hrp.Parent then break end; hrp.CFrame=tCF; game:GetService("RunService").Heartbeat:Wait()end; hrp.Anchored=false end); pcall(function()h:ChangeState(Enum.HumanoidStateType.Running)end); if not s then Rioo:Notify({Title="TP Err",Content="Err: "..tostring(e)})end; task.wait(0.1); if(hrp.Position-tCF.Position).Magnitude>20 then Rioo:Notify({Title="TP FAIL",Content="Rubberband."})end end
local function TeleportToIsland(iN) local cf=IslandCFrames[iN]; if not cf then Rioo:Notify({Title="TP Err",Content="CFrame?"}); return end; Rioo:Notify({Title="TP",Content="To "..iN}); TeleportBrutal(cf) end
local function TeleportToPlayer(pN) local p=game.Players:FindFirstChild(pN); if not p then Rioo:Notify({Title="TP Err",Content="Player?"}); return end; if not p.Character or not p.Character:FindFirstChild("HumanoidRootPart")then Rioo:Notify({Title="TP Err",Content="Char?"}); return end; local cf=p.Character.HumanoidRootPart.CFrame; Rioo:Notify({Title="TP",Content="To "..pN}); TeleportBrutal(cf) end
local function RespawnNow() local p=game.Players.LocalPlayer; local c=p.Character; if c then c:BreakJoints(); p:LoadCharacter(); p.CharacterAdded:Wait(); Rioo:Notify({Title="Respawn",Content="OK"}); TeleportBrutal(IslandCFrames["Fisherman Island"])else Rioo:Notify({Title="Respawn Err",Content="Char?"})end end
local function ToggleFloating(s) floatingActive=s; if s then Rioo:Notify({Title="Floating",Content="ON"}); floatLoop=game:GetService("RunService").Heartbeat:Connect(function()if not floatingActive then return end; local c=game.Players.LocalPlayer.Character; if c and c:FindFirstChild("Humanoid")then c.Humanoid:ChangeState(Enum.HumanoidStateType.Physics)end end)else Rioo:Notify({Title="Floating",Content="OFF"}); if floatLoop then floatLoop:Disconnect();floatLoop=nil end; local c=game.Players.LocalPlayer.Character; if c and c:FindFirstChild("Humanoid")then c.Humanoid:ChangeState(Enum.HumanoidStateType.Running)end end end
local function ToggleFishingRadar(s) if RemoteUpdateFishingRadar then pcall(function()RemoteUpdateFishingRadar:InvokeServer(s)end); Rioo:Notify({Title="Radar",Content=s and "ON" or "OFF"})else Rioo:Notify({Title="Radar Err",Content="Remote?"})end end
local function ToggleDarkMode(s) if s then Rioo:Notify({Title="Dark",Content="ON"}); game.Lighting.ClockTime=0; game.Lighting.Brightness=1; game.Lighting.Ambient=Color3.fromRGB(20,20,20)else Rioo:Notify({Title="Dark",Content="OFF"}); game.Lighting.ClockTime=originalLighting.ClockTime; game.Lighting.Brightness=originalLighting.Brightness; game.Lighting.Ambient=originalLighting.Ambient end end
local function ToggleAntiLag(s) if s then Rioo:Notify({Title="AntiLag",Content="ON"}); game.Lighting.GlobalShadows=false; game.Lighting.FogEnd=999999; settings().Rendering.QualityLevel=Enum.QualityLevel.Level01 else Rioo:Notify({Title="AntiLag",Content="OFF"}); game.Lighting.GlobalShadows=originalLighting.GlobalShadows; game.Lighting.FogEnd=originalLighting.FogEnd; settings().Rendering.QualityLevel=originalQuality end end
local function ToggleDisableCutscene(s) disableCutsceneActive=s; Rioo:Notify({Title="NoCutscene",Content=s and "ON" or "OFF"})end
local function ToggleLockPosition(s) if s then local p=game.Players.LocalPlayer; local c=p.Character; if not c or not c:FindFirstChild("HumanoidRootPart")then Rioo:Notify({Title="Lock Err",Content="Char?"}); return end; savedPosition=c.HumanoidRootPart.CFrame; Rioo:Notify({Title="Lock",Content="ON"}); lockPosLoop=game:GetService("RunService").Heartbeat:Connect(function()local p=game.Players.LocalPlayer; local c=p.Character; if not c or not c:FindFirstChild("HumanoidRootPart")then ToggleLockPosition(false); return end; c.HumanoidRootPart.CFrame=savedPosition end)else Rioo:Notify({Title="Lock",Content="OFF"}); if lockPosLoop then lockPosLoop:Disconnect();lockPosLoop=nil end end end
local function ToggleAutoTeleportEvent(s) autoTeleportEventActive=s; if s then local kw=EventKeywords[selectedEventName]; if not kw then Rioo:Notify({Title="Event Err",Content="Keyword?"}); return end; Rioo:Notify({Title="Event Scan",Content="Scan '"..kw.."'..."}); eventScanLoop=game:GetService("RunService").Heartbeat:Connect(function()if not autoTeleportEventActive then return end; pcall(function()for _,c in ipairs(game:GetService("Workspace"):GetChildren())do if string.find(string.lower(c.Name),kw)then Rioo:Notify({Title="Event Found!",Content="TP to "..c.Name}); local cf=c:GetPrimaryPartCFrame(); task.spawn(function()TeleportBrutal(cf)end); break end end end)end)else Rioo:Notify({Title="Event Scan",Content="Stop"}); if eventScanLoop then eventScanLoop:Disconnect();eventScanLoop=nil end end end

-- ====================
-- FUNGSI BUAT BIKIN UI UTAMA (BARU)
-- ====================
local function CreateMainWindow()
    local Window = Rioo:CreateWindow({
        Name = "Rioo Hub Project", -- Ganti Nama
    ...
    -- TAB 6: SHOP
    -- ====================
    local ShopTab = Window:CreateTab("Shop", 4483362458); local BoatsSection=ShopTab:CreateSection("Buy Boats"); local boats={{name="Small Boat",id=1},{name="Kayak",id=2},{name="Jetski",id=3},{name="Highfield Boat",id=4},{name="Speed Boat",id=5},{name="Fishing Boat",id=6},{name="Mini Yacht",id=7}}; for _,b in ipairs(boats)do ShopTab:CreateButton({Name="Buy "..b.name,Callback=function()pcall(function()task.spawn(function()if RFPurchaseBoat then RFPurchaseBoat:InvokeServer(b.id); Rioo:Notify({Title="Shop",Content="Purchased "..b.name.."!",Duration=3,Image=4483362458})else Rioo:Notify({Title="Shop Err",Content="Remote?"})end end)end)end})end; local GearsSection=ShopTab:CreateSection("Buy Gears"); ShopTab:CreateButton({Name="Buy Fishing Radar",Callback=function()pcall(function()task.spawn(function()if RFPurchaseGear then RFPurchaseGear:InvokeServer(81); Rioo:Notify({Title="Shop",Content="Radar OK",Duration=3,Image=4483362458})else Rioo:Notify({Title="Shop Err",Content="Remote?"})end end)end)end}); ShopTab:CreateButton({Name="Buy Diving Gear",Callback=function()pcall(function()task.spawn(function()if RFPurchaseGear then RFPurchaseGear:InvokeServer(105); Rioo:Notify({Title="Shop",Content="Diving OK",Duration=3,Image=4483362458})else Rioo:Notify({Title="Shop Err",Content="Remote?"})end end)end)end}); local WeatherSection=ShopTab:CreateSection("Buy Weather"); local weathers={"Cloudy","Snow","Storm","Radiant","SharkHunt","Wind"}; for _,w in ipairs(weathers)do ShopTab:CreateButton({Name="Buy "..w,Callback=function()pcall(function()task.spawn(function()if RFPurchaseWeatherEvent then RFPurchaseWeatherEvent:InvokeServer(w); Rioo:Notify({Title="Shop",Content=w.." OK",Duration=3,Image=4483362458})else Rioo:Notify({Title="Shop Err",Content="Remote?"})end end)end)end})end; local RodsSection=ShopTab:CreateSection("Buy Fishing Rods"); local rods={{name="Luck Rod",id=79},{name="Carbon Rod",id=76},{name="Grass Rod",id=85},{name="Damascus Rod",id=77},{name="Ice Rod",id=78},{name="Lucky Rod",id=4},{name="Midnight Rod",id=80},{name="Steampunk Rod",id=6},{name="Chrome Rod",id=7}}; for _,r in ipairs(rods)do ShopTab:CreateButton({Name="Buy "..r.name,Callback=function()pcall(function()task.spawn(function()if RFPurchaseFishingRod then RFPurchaseFishingRod:InvokeServer(r.id); Rioo:Notify({Title="Shop",Content=r.name.." OK",Duration=3,Image=4483362458})else Rioo:Notify({Title="Shop Err",Content="Remote?"})end end)end)end})end

    -- Final Notification
    local notifySuccess, notifyErr = pcall(function()
        Rioo:Notify({
            Title = "Rioo Hub Loaded!",
...
local KeyWindow = Rioo:CreateWindow({
    Name = "Rioo Hub",
    LoadingTitle = "Rioo Hub",
    ...
    Callback = function()
        ...
        local success, err = pcall(function()
            ...
            if not enteredKey or enteredKey:match("^%s*$") then -- Cek spasi doang juga
                Rioo:Notify({Title="Error", Content="Key kosong!", Duration=5})
                return
            end
            ...
            if isValid then
                ...
                Rioo:Notify({ Title = "Success", Content = "Key valid! Akses diberikan.", Duration = 5, Image = 4483362458 })
                ...
                -- Key invalid
                Rioo:Notify({ Title = "Error", Content = "Key tidak valid. Coba lagi.", Duration = 5, Image = 4483362458 })
            end
        end)
        if not success then
            warn("Callback error in Submit Key:", err)
            Rioo:Notify({Title="Callback Error", Content="Ada error di callback. Cek console.", Duration=5})
        end
    end,
})
